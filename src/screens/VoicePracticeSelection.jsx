...
const VoicePracticeSelection = ({ gradeLevel, onSelectScenario, onBack, darkMode = true }) => {
  ...
  const normalizedGrade = useMemo(() => normalizeGradeLevel(gradeLevel || '6-8'), [gradeLevel]);
    ...
    const fetchScenarios = async () => {
      ...
      try {
        ...
        if (data.success && data.scenarios) {
          // Convert API scenarios to format compatible with our data structure
          const formattedScenarios = data.scenarios.map(apiScenario => {
            // Try to get full scenario data from our local database
            const localScenario = getScenarioForGrade(apiScenario.id || apiScenario.name, normalizedGrade);
            
            if (localScenario) {
              return {
                ...localScenario,
                id: localScenario.id || apiScenario.id,
                icon: scenarioIcons[localScenario.id] || '💬',
                categoryLabel: formatCategoryLabel(localScenario.category)
              };
            }
            ...
            return {
              ...
              category: apiScenario.category || 'general',
              categoryLabel: formatCategoryLabel(apiScenario.category || 'general'),
